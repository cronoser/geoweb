function peticionUnoApiGlobal() {
    ...
    var url_servidor;
    for (var i = 0; i < options.length; i++) { //recorre el array
        if (options[i].checked) { //si encuentra un valor con checked lo guarda
    ...
    var peticion1 = url_servidor + "q=" + textoBuscar + "&limit=" + limiteResultados + "&only=map"; 
        //concatena en peticion1 los valores urlsevidor textobuscar y los resultados SOLO MAPA
...
async function peticionDosObtenerRecurso(data) {
  ...
  return await  enviarPeticion(peticion2).then(function (respuestaNodoSocrata) {
        ...
        if (respuestaNodoSocrata.metadata && respuestaNodoSocrata.metadata.geo) { //si tiene.geo es geojson

            urlRecurso = 'https://' + params[1] + '/api/geospatial/' + respuestaNodoSocrata.childViews[0] + '?method=export&format=GeoJSON'; //crea una variables por si tenemos que tratarlo con geojson
            isGeojson = true;
            bbox = respuestaNodoSocrata.metadata.geo.bbox; //si es geojson crea una caja de coordenadas
        } else { // es una tabla que la devuelve en formato geogjon
...
} //finfuncion peticionDos

function peticionTresObtenerDato(urlRecurso, bbox, isGeojson) {

    enviarPeticion(urlRecurso).then(function (respuestaRecurso) { //pido el dato o recurso
        ...
        if (isGeojson) { 
            ...
            for (var i = 0; i < respuestaRecurso.length; i++) { //recorro la tabla que me has devuelto

                if (respuestaRecurso[i].location_1) { //si existe el location calculo el long lat
    ...
    }) //fin peticionTresObtenerDato
}//fin funcion peticionTresObtenerDato
