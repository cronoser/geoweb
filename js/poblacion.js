...
function getBounds() { //captura la caja de coordenadas que vemos en el momento en la pantalla
...
async function getFeaturesFGB(urlDatos, minZoom) { //el zoom ajusta los datos que devuelve para no descargar todos los datos a la vez
    ...
    if (!map.getSource("mallaH3")) {
        ...
        map.addLayer({ 
            ...
            minZoom: minZoom,
            paint: {
                "fill-color": "#0000ff", //azul
                "fill-outline-color": "#ffffff", //blanco
                "fill-opacity": 0.1,  //transpariencia. cambiando el valor entre 0 y 1, la malla apenas se vé en 0.1 por ejemplo
            },
        });
...
async function calculaPoblacion(geometriaMapa) {
    ...
    for (var i = 0; i < datosH3Geojson.features.length; i++) { //recorre los hexagonos que están dentro o en la intersección
        ...
        if (turf.booleanWithin(datosH3Geojson.features[i], geometriaMapa)) { 
            ...
            var poblacionHexagono = datosH3Geojson.features[i].properties.population; //poblacion Total
            var areaHexagono = turf.area(datosH3Geojson.features[i]); //area total hexagono
            var geometriaIntersecion = turf.intersect(datosH3Geojson.features[i], geometriaMapa);
            var areaIntersecion = turf.area(geometriaIntersecion); //area calculada
            ...
            */
            poblacion = poblacion + parseInt(areaIntersecion * poblacionHexagono / areaHexagono); //sacamos la regla de 3 para calcular la población proporcional al area seleccionada
...
function gestionCalculoPoblacion(feature,minZoom){ //esta función recibe un feature y un minzoom

    if (minZoom <= map.getZoom()) {

        if(feature.geometry.type =="Polygon"){ //para polígonos
           ...
           if(feature.geometry.type =="Point"){ //para puntos
            creaBuffer(feature,1000, 'meters').then(function(bufferFeature){ //cuando esté creado el buffer de 1000 metros que es un poligono
